...
目录：
1. 什么是shell：shell是命令解释器，也就是将我们执行的命令翻译给内核，然后将我们命令执行的结果返回给用户
   1. 例子：比如我们自己输入`ls`这个命令，首先由用户接收到这个命令，之后会对命令的参数和选项进行分析，分析之后交给我们的内核的文件系统，然后内核接收到这个命令，然后翻译到对应的扇区，然后将结果返回内核，内核将结果返回给用户
2. Linux的启动过程涉及到的shell
   1. 系统自带的shell脚本有两大功能：第一是用于Linux系统的启动，第二是Linux命令用shell脚本编写出来
   2. Linux启动过程：
3. 怎样编写shell脚本
4. 编写自己的shell脚本：shell脚本的编写方式
5. 内建命令和外建命令的区别：脚本中创建子进程和不创建子进程
