...
线段树的维护只需要用小区间的值更新大区间的值，另外线段树是二叉平衡树，所以时间复杂度总是log级别的
使用线段树解决的问题需要满足区间加法，也就是对于一个[l,r]的区间，它的答案可以由[l,m]和[m+1,r]的答案合并给出，这里的M是区间中点，
满足的问题：区间求和，区间最大最小值问题等
不满足的问题：区间的众数，区间最长连续问题，最长不下降问题等

线段树解决问题的步骤：
1. 建树：开一个数字，以堆的方式存储数据
2. 单点修改，区间修改
3. 区间查询


区间修改以及区间修改后的查询会使用到lazy标记
